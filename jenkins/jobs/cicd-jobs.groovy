...
// repo), not from the application being built:
//
//   * cicd-build              -> lightweight Jenkinsfile fetch. The Jenkinsfile
//                                sets skipDefaultCheckout(true) and clones the
//                                caller-supplied GIT_URL itself, so the job is
//                                repo-agnostic.
//   * cicd-publish-nexus-*    -> FULL checkout of this repo. These pack/build the
//                                source referenced by build-info.json (BUILD_FILE,
//                                e.g. src/app/cicd.sln, which lives here), so they
//                                need the tree in the workspace. (Making publish
//                                repo-agnostic too is a follow-up: give those
//                                Jenkinsfiles the same skipDefaultCheckout + GIT_URL
//                                checkout treatment as cicd-build.)
//
